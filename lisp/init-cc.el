...
(with-eval-after-load "cc-mode"
  (defun c-page-break ()
    "Insert a multi-line page break section comment."
    (interactive)
    (let* ((comment (read-from-minibuffer "comment: "))
      ...
      (let* ((indent (current-column))
             (line-width (- width indent prefix-len))
             (sep-line (make-string line-width ?=))
             (padding (max 0 (/ (- line-width (length comment)) 2)))
             (comment-line (concat (make-string padding ? ) comment)))
        (insert prefix sep-line "\n")
        (indent-to indent)
        (insert prefix comment-line "\n")
        (indent-to indent)
        (insert prefix sep-line))))
